...
public class AStar {
	...
	//METODO PRINCIPAL DO ALGORITMO A*
	public static List<Node> findPath(World world, Vector2i start, Vector2i end){	/*Pega os parametros dos tiles no World, a posicao inicial e
		posicao final*/
		lastTime = System.currentTimeMillis();
		List<Node> openList = new ArrayList<Node>();	//openList eh uma array com as posicoes possiveis que Enemy vai percorer
		List<Node> closedList = new ArrayList<Node>();	//closeList eh o array de posicoes nao validas para o Enemy
		...
		while(openList.size() > 0) {
			...
			if(current.tile.equals(end)) {	//se chegar no destino final
			...
			//VERIFICARCAO DAS POSICOES AO REDOR DO ENEMY
			for(int i = 0; i < 9; i++) {
				if(i == 4) continue;	// 4 eh a posicao inicial do Enemy
				int x = current.tile.x;
				int y = current.tile.y;
				int xi = (i%3) - 1;
				int yi = (i/3) - 1;
				//VERIFICACAO DO TIPO DE TILE
				Tile tile = World.tiles[x + xi + ((y + yi) *World.WIDTH )];
				if (tile == null) continue;
				if (tile instanceof WallTile) continue;
				//ANDAR EM TODAS A DIRECOES 
				if(i == 0) {
					...
					//ele testa se tem como andar nadiagonal na posicao 6
	...
	//CALULCAR DISTANCIA DE UM TILE AO OBJETIVO
	private static double getDistance(Vector2i tile, Vector2i goal) {
		//esse metodo eh muito parecido com o CALCULO DE DISTANCIA DE ENTITIES
